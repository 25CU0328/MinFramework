...
bool RenderManager::Init(const HWND* _pHwnd)
{
	...
	if (!m_rootSignature.Init(m_pDevice.Get(), 1))
	{
		return false;
	}
...
// デバイスの初期化処理
HRESULT RenderManager::_initDevice()
{
...
// バックバッファーの初期化処理
HRESULT RenderManager::_initBackBuffer()
{
	// バックバッファーを宣言
...
// デバイスを取得する
ID3D12Device* RenderManager::GetDevice()
{
...
// ディスクリプタを取得する
DescriptorHeap& RenderManager::GetDescriptorHeap()
{
	return m_mainHeap;
}

ID3D12GraphicsCommandList* RenderManager::GetCommandList()
{
